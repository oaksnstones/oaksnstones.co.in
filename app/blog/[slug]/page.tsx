...
export async function generateStaticParams() {
  return (Array.isArray(blogPosts) ? blogPosts : []).map(p => ({ slug: p.slug }))
}
...
export default async function BlogPostPage({ params }: Props) {
  ...
  const idx = (Array.isArray(blogPosts) ? blogPosts : []).findIndex(p => p.slug === slug)
  const prev = blogPosts[idx - 1]
  const next = blogPosts[idx + 1]
  const related = (Array.isArray(blogPosts) ? blogPosts : []).filter(p => p.slug !== slug).slice(0, 3)
